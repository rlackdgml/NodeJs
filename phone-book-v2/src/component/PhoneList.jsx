...
const PhoneList = ({
  ...
  /**
   * Hook 함수
   * use* 로 시작되는 함수들(useState, useEffect, useReducer)
   *
  ...
  /*
    ...
    이함수는 여러가지 용도로 설정하여 만들수 있다
    이 함수가 호출될때 어떠한 일을 할지 설정하는 속성이 action.type 있다
    즉, action.type 값을 임의로 정하여 reducer함수가 작동되는 방법을 정의 할수 있다
  */
  const reducer = (object, action) => {
    // 만약 action.type 이 EDIT_FORM으로 설정되어서 reducer가 호출되면 실행될 코드
    if (action.type === "EDIT_FORM") {
      return { name: action.name, number: action.number };
    }
    // input box에 문자열을 입력하는 change이벤트가 발생할때 default로 실행될코드
