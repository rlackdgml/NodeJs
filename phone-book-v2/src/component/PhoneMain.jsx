...
function PhoneMain(props) {
  ...
  /**
   * ref 형 변수는 선언고 동시에 .current 라는 속성이 만들어지고
  ...
  const insertPhoneBook = (name, number) => {
    /**
     *  표준 js에서 배열요소를 추가할떄 push() 라는 함수를 사용한다
     * react에서는 기존의 배열요소에 어떤값을 추가하는 것이 금지되어있다
     * 기존배열 + 추가될요소를 새로운 배열로 만들고 그배열을 기존배열로
     * 대치하는 방법을 사용해야한다.
     */
    setPhoneBooks([
      ...phoneBooks,
  ...
  useEffect(() => {
    // 추가된 데이터가 있으면 전체 phoneBooks를 브라우저의 localStrage에 저장
    // 1. 배열 데이터를 문자열화 시키기
