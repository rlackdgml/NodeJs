...
/**
 * myarry 요소를 foreach문으로 반복하는 것과 같이 순회하면서
 ...
 * 연산을 수행한 후 다른 변수에 return 하여 값을 담는 구조
 */
const mapArr = myarray.map((arr) => {
  // arr++; // map 함수내부에서 myarry의 요소를 변경하는 코드는
  // 가급적 지양하라  순수함수
...
/**
 * 원본배열에서 index(3) 위치의 값을 100 으로 변경하여 새로운 배열 생성
 * 이때도 원본배열은 변경되지 않는다.
 */
const index = 3;
const updateArr = [
